...
from pyrpc import PyRPCRouter, PyRPCFastAPI, PyRPCError
...
def test_successful_request(client):
    ...
    response = client.post(
        "/api/query/divide",
        json={"input": {"numerator": 10, "denominator": 2}}
    )
...
def test_bad_request(client):
    ...
    response = client.post(
        "/api/query/divide",
        json={"input": {"numerator": 10, "denominator": 0}}
    )
...
def test_unauthorized(client):
    ...
    response = client.post(
        "/api/query/protected",
        json={"input": {"token": "invalid_token"}}
    )
