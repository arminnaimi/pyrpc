...
from pyrpc import PyRPCRouter, PyRPCFastAPI, PyRPCFlask, PyRPCDjango, PyRPCError, PyRPCContext
import asyncio
...
def context_router():
    ...
    def context_test_mutation(input: ContextInput, context: PyRPCContext) -> ContextOutput:
        try:
            context_value = context.raw_request["test_value"]
        except (KeyError, TypeError, AttributeError):
            context_value = "default"
...
async def test_django_view_direct(router):
    ...
    response = await view(request, path="echo")
    ...
    class PyRPCView(View):
        async def post(self, request, path: str):
            try:
                ...
                result = await pyrpc.handle_request(path, data.get("input"), context=context)
                return JsonResponse({
                    "result": result.model_dump(),
                    "success": True
                })
...
def test_django_query_with_context(django_app_with_context, context_router):
    ...
    request.pyrpc_context = PyRPCContext(raw_request={"test_value": "from_middleware"})
    ...
    class PyRPCView(View):
        ...
        async def post(self, request, path: str):
            try:
                ...
                result = await self.pyrpc.handle_request(path, data.get("input"), context=context)
        ...
        async def dispatch(self, request, *args, **kwargs):
            ...
            try:
                ...
                print(f"Dispatch error: {str(e)}")  # Debug print
                import traceback
                traceback.print_exc()  # Print full traceback
                return JsonResponse({
                    "success": False,
    ...
    view = PyRPCView.as_view()
    response = asyncio.run(view(request, path="context_test"))
...
async def test_django_view_error_handling(error_router):
    ...
    response = await view(request, path="error")
    assert response.status_code == 400
    data = json.loads(response.content.decode())
    assert data["success"] is False
    assert data["error"]["code"] == "TEST_ERROR"
