...
This module implements the core functionality of PyRPC, providing a type-safe RPC framework
for Python applications. It includes the router implementation, procedure definitions,
...
from typing import TypeVar, Generic, Callable, Optional, Type, Self, Any, Union, get_args
...
class ProcedureDef(Generic[Input, Output]):
    ...
    Args:
        ...
        output_model (Type[Output]): Pydantic model class for procedure output validation
        is_mutation (bool): Whether this procedure modifies state (default: False)
...
class ProcedureBuilder(Generic[Input, Output]):
    ...
    def __init__(
        ...
        self._resolver: Optional[Callable[[Input, PyRPCContext], Output]] = None
    ...
    def __call__(self, fn: Callable[[Input], Output] | Callable[[Input, PyRPCContext], Output]) -> Callable:
        ...
        if not input_type or not output_type:
            raise PyRPCError("VALIDATION", "Function must have input and output type hints")
...
class PyRPCRouter:
    ...
    async def handle(self, path: str, input_data: Any, context: Optional[PyRPCContext] = None) -> Any:
